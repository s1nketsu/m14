...
struct CorrectRecordModel {
    let title: String
    let publishedAtDate: Date
}

struct cellModel {
    let cellPublishedAt: Date
    let cellTitle: [CorrectRecordModel]
}

let newsModels: [NewsRecordModel] = [
    NewsRecordModel(
...
//MARK: - Обработка модели

func cropDate(date: String) -> Date {
    let calendar = Calendar.current
    let dateFormatter = DateFormatter()
    dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.000Z"
    let dateFromString = dateFormatter.date(from: date) ?? Date.now
    let year = calendar.component(.year, from: dateFromString)
    let month = calendar.component(.month, from: dateFromString)
    let day = calendar.component(.day, from: dateFromString)
    ...
    let correctDate = calendar.date(from: dateComponents) ?? Date.now
    return correctDate
}

let newsModelsWithDate: [CorrectRecordModel] = newsModels.map { NewsRecordModel in
    return CorrectRecordModel(title: NewsRecordModel.title, publishedAtDate: cropDate(date: NewsRecordModel.publishedAt))
}

let dictModelsWithDate = Dictionary(grouping: newsModelsWithDate) { $0.publishedAtDate }

var almostFinishArray = dictModelsWithDate.map { (key: Date, value: [CorrectRecordModel] ) in
    return cellModel(cellPublishedAt: key, cellTitle: value)
}

let finishArray = almostFinishArray.sorted { $0.cellPublishedAt < $1.cellPublishedAt }
